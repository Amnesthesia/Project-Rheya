...
class Mouth
  ...
  attr_accessor :debug, :db, :question_starters, :previously_said, :emotions, :last_poll_id, :poll_db
  debug = true
  ...
  def construct_sentence(msg, predef_context = [])
    ...
    begin
      ...
      # Append a randomly chosen word to our semi-constructed sentence
      ...
      prev_word = get_word(remember_previous_word[:word])
  ...
  #
  def get_poll(id = nil)
    id = @last_poll_id if id.nil?
    puts "Trying to get poll with id "+id.to_s
    if id.nil?
      id = "(SELECT id FROM polls ORDER BY id DESC LIMIT 1)"
    end
